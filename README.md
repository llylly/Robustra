The repo contains source code of the paper [Robustra](https://www.ijcai.org/proceedings/2019/654).

Built based on Wong and Zico's dual LP training framework, we do several major extensions:

- Transferability-based dual training (see convex_adversarial/dual_network.py, examples/transfer_mnist.py)

- Adaptor for model weight transformation for verification on MILP solver proposed by Tjeng et al (arXiv: 1711.07356, ICLR 2019)

- Our trained model weights in final_models/ (the naming corresponds to estimated LP bound, and the exact number for each model will be given later)
