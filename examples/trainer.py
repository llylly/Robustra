...
from convex_adversarial import robust_loss, robust_loss_parallel, robust_loss_transfer
...
def evaluate_robust(loader, model, epsilon, epoch, log, verbose, 
    ...
    for i, (X,y) in enumerate(loader):
        ...
        if y.dim() == 2: 
            y = y.squeeze(1)
        robust_ce, robust_err = robust_loss(model, epsilon, X, y, parallel=True, **kwargs)
    ...
    torch.cuda.empty_cache()
    print('')
    print(' * Robust error {rerror.avg:.4f}\t'
          'Error {error.avg:.4f}'
          .format(rerror=robust_errors, error=errors))
    return robust_errors.avg


def robust_transfer(loader, model, t_model, opt, epsilon, epoch, log, verbose,
                    real_time=False, evaluate=False, clip_grad=None,
                    M2_batch_mem_lim=None, **kwargs):
    ...
    robust_losses = AverageMeter()
    robust_errors = AverageMeter()
    robust_m2_losses = AverageMeter()
    robust_m2_errors = AverageMeter()
    exact_ratios = AverageMeter()
    relax_ratios = AverageMeter()
    invalid_ratios = AverageMeter()
    neg_etas = AverageMeter()
    ...
    for i, (X,y) in enumerate(loader):
        ...
        # by default, size_average=True, so weighted average is used here
        if M2_batch_mem_lim is None or M2_batch_mem_lim > X.size(0):
            M2_batch_mem_lim = X.size(0)
        orig_size = X.size(0)
        if not evaluate:
            opt.zero_grad()
        ...
        while True:
            nowX = X[:M2_batch_mem_lim]
            nowy = y[:M2_batch_mem_lim]
            nows = nowX.size(0)
            now_robust_ce, now_robust_err, now_robust_ce_orig, now_robust_err_orig, now_exact_ratio, now_relax_ratio, now_invalid_ratio, now_neg_eta = \
                robust_loss_transfer(model, t_model, epsilon,
                                     Variable(nowX), Variable(nowy),
                                     evaluate=evaluate,
                                     **kwargs)
            if not evaluate:
                (now_robust_ce * float(nows) / float(orig_size)).backward()
            robust_ce += now_robust_ce.detach().item() * nows
            robust_err += now_robust_err * nows
            robust_ce_orig += now_robust_ce_orig.detach().item() * nows
            robust_err_orig += now_robust_err_orig * nows
            exact_ratio += now_exact_ratio * nows
            relax_ratio += now_relax_ratio * nows
            invalid_ratio += now_invalid_ratio * nows
            neg_eta += now_neg_eta * nows
            if nows >= X.size(0):
                break
            else:
                X = X[M2_batch_mem_lim:]
                y = y[M2_batch_mem_lim:]

        robust_ce /= orig_size
        robust_err /= orig_size
        robust_ce_orig /= orig_size
        robust_err_orig /= orig_size
        exact_ratio /= orig_size
        relax_ratio /= orig_size
        invalid_ratio /= orig_size
        neg_eta /= orig_size
        ...
        losses.update(ce.item(), X.size(0))
        errors.update(err.item(), X.size(0))
        robust_losses.update(robust_ce, X.size(0))
        robust_errors.update(robust_err, X.size(0))
        robust_m2_losses.update(robust_ce_orig, X.size(0))
        robust_m2_errors.update(robust_err_orig, X.size(0))
        exact_ratios.update(exact_ratio, X.size(0))
        relax_ratios.update(relax_ratio, X.size(0))
        invalid_ratios.update(invalid_ratio, X.size(0))
        neg_etas.update(neg_eta, X.size(0))
        ...
        print(epoch, i,
              robust_ce, robust_err,
              robust_ce_orig, robust_err_orig,
              ce.item(), err.item(),
              exact_ratio, relax_ratio, invalid_ratio, neg_eta,
              file=log)
        ...
        if not evaluate:
            if verbose and (i % verbose == 0 or real_time):
                ...
                print('Epoch: [{0}][{1}/{2}]\t'
                      'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t'
                      'Exct,Rlx,Ivld {exact_ratio.val:.3f},{relax_ratio.val:.3f},{invalid_ratio.val:.3f} ({exact_ratio.avg:.3f},{relax_ratio.avg:.3f},{invalid_ratio.avg:.3f})\t'
                      'NegEta {neg_eta.val:.3f} ({neg_eta.avg:.3f})\t'
                      'T.R. loss {rloss.val:.4f} ({rloss.avg:.4f})\t'
                      'T.R. err {rerrors.val:.3f} ({rerrors.avg:.3f})\t'
                      'R. loss {r2loss.val:.4f} ({r2loss.avg:.4f})\t'
                      'R. err {r2errors.val:.3f} ({r2errors.avg:.3f})\t'
                      'Loss {loss.val:.4f} ({loss.avg:.4f})\t'
                      'Err {errors.val:.3f} ({errors.avg:.3f})'.format(
                       epoch, i, len(loader), batch_time=batch_time,
                       # data_time=data_time,
                       exact_ratio=exact_ratios, relax_ratio=relax_ratios, invalid_ratio=invalid_ratios,
                       neg_eta=neg_etas,
                       rloss=robust_losses, rerrors=robust_errors,
                       r2loss=robust_m2_losses, r2errors=robust_m2_errors,
        ...
        del X, y, robust_ce, out, ce, err, robust_err
        del robust_ce_orig, robust_err_orig
        if DEBUG and i == 10:
            break
    print('')
    torch.cuda.empty_cache()
