...
def select_input(X, epsilon, proj, norm, bounded_input, favor_l1=False):
    # Not use projected input layer in transfer training. Because transfer training requires gradient, which only varies on input layer
    # Therefore, accurate input layer is needed.
    if proj is not None and norm=='l1_median' and X[0].numel() > proj and not favor_l1:
        if bounded_input:
            return InfBallProjBounded(X,epsilon,proj)
        else: 
            return InfBallProj(X,epsilon,proj)
    elif norm == 'l1' or (norm=='l1_median' and favor_l1):
        if bounded_input: 
