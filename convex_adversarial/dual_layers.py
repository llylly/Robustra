...
def select_layer(layer, dual_net, X, proj, norm_type, in_f, out_f, zsi,
                 zl=None, zu=None, transfer_training=False):
...
class DualLinear(DualLayer): 
    ...
    def apply(self, dual_layer):
        # ###: Although bias term is continuously appended, but in objective function,
        # ###: we know only the first term, namely, b_i, is used.
        # ###: but in bounds function, the last term of b_i, which is b_i*D_(i+1)*W_(i+1)*...*(b_now)*(W_now)
        # ###: is added to bounds
